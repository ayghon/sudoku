...
import { generateKeysAndClean, transformObjectToKeys } from './i18n-generator.utils';
...
targetPaths.forEach((targetPath) => generateKeysAndClean(targetPath, TRANSLATIONS_FILE_PATH));
...
const printableTranslationKeys = JSON.stringify(translationKeys, null, 2)
  .replace(/:\s?"(.*)",?\s/g, (string, groupMatching) => `: '${groupMatching}',\n`)
